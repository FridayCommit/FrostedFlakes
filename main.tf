...
data "http" "set_branch" {
  for_each        = module.repo
  url             = "${var.sonar_url}/api/project_branches/rename?name=${data.github_repository.repo_data[each.value.repo_instance.name].default_branch}&project=${each.value.repo_instance.name}"
  //TODO felix fatta hur vi passar branch
  method          = "POST"
  request_headers = {
...
data "http" "set_alm" {
  for_each        = module.repo
  url             = "${var.sonar_url}/api/alm_settings/set_github_binding?almSetting=GitHub&monorepo=no&project=${each.value.repo_instance.name}&repository=${each.value.repo_instance.full_name}"
  //TODO felix fatta hur vi passar branch
  method          = "POST"
  request_headers = {
    Authorization = "Basic ${base64encode(format("%s:",var.sonar_admin_token))}"
